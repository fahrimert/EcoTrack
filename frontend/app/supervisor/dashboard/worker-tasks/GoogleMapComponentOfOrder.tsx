...
const GoogleMapComponentOfOrder = ({session} : {session: RequestCookie | undefined}) => {
 ...
 const { source ,setSource} = useContext(WorkerLocationContext);
  const { destination ,setDestination} = useContext(SensorDestinationContext);
...
export default GoogleMapComponentOfOrder
