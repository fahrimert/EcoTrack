...
import { TaskDetail } from "./AssignedTaskFormForSolving";
import { TaskSensorWithTask } from "../../../components/SensorComponents/SensorsAndMap";

//bu kısımda not ekliyip gidiyorum şu dakkaya ordayım diyebilmeli böylece butonla arriving kısmını ayarlicam işte 
...
const AssignedSensorFormForOnRoad= ({initialData} : {initialData: 
 TaskSensorWithTask}) => {
...
<div className= {cn(`w-full h-full absolute inset-0 z-10 flex items-center justify-center bg-black text-lg font-bold text-white bg-opacity-50 ${initialData.workerArrived && "pointer-events-none "}`)}>
    Yoldayım notu zaten girildi
  </div>
    }

    {/* ya mantık eğer work arriving true olmuşsa ve work arrived olmamışsa bunu blurla */}
<div className={cn(`flex flex-col gap-[5px] w-full  rounded-[5px]  ${initialData.workerArriving && !initialData.workerArrived   && "blur-sm  pointer-events-none"}`)}>
...
<h3 className=" w-full h-fit text-[24px] font-semibold">Yoldayım Notu</h3>
                ...
                <FormField
                 ...
                 render = {({field}) => (
                    <FormItem>
                        ...
                        <FormControl>
                            <Textarea disabled = {loading} placeholder="Arıza ile alakalı yoldayım notu ekleyin" {...field}/>
                        </FormControl>
                        <FormMessage/>
                    </FormItem>
)}
